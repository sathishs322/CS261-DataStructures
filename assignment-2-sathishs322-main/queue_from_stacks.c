...
struct queue_from_stacks* queue_from_stacks_create() {
 struct queue_from_stacks* q_from_s=malloc(sizeof(struct queue_from_stacks));
 q_from_s->s1=stack_create();
 q_from_s->s2= stack_create();
  return q_from_s;
}
...
void get_s2(struct queue_from_stacks* qfs){
   while (stack_isempty(qfs->s1)!=1){
     void* val= stack_pop(qfs->s1);
      stack_push(qfs->s2,val); 
   }   
}
void get_s1(struct queue_from_stacks* qfs){
